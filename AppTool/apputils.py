...
class apputils(object):
    def __init__(self, uuid:str = None, bundle_id:str = None, parent_dir=os.environ["HOME"]):
        """sumary_line
        ...
        """
        self.__client = wda.USBClient(udid=uuid)
        self.__logData = {}
        self.__uuid = uuid
        ...
        if not self.__bundle_id:
            raise ValueError("invalid bundle_id value")

        # mount point path
        self.__root_dir = os.path.expanduser(parent_dir)
        
        #/mount_parent_dir/uuid_bundleId_documents
        self.__mount_documents_path = os.path.join(self.__root_dir, uuid + bundle_id + "documents")
        self.__mount_container_path = os.path.join(self.__root_dir, uuid + bundle_id + "container")
    ...
    def creat_dir_if_not_exist(self, folder_path):
    ...
    def push_mock_interface(self, url_path: str = None, file_path: str = None):
        ...
        if len(url_path) == 0 or len(file_path) == 0:
            raise ValueError("push_mock_interface arguments not empty")
